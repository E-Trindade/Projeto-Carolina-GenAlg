nomes = ['Cláudio', 'Josefina', 'Lúcio', 'Maria Clara', 'Josifredo',
         'Petúnia', 'Ricardo', 'Rosefrilde', 'Poroshenko', 'Lucashenko', 'Pudjin']
import random
import math
...
class objeto_fisico(object):
    ...
    def mover(self, t):
        # Movendo o objeto
        vx, vy = math.cos(self.velocidade.ang) * self.velocidade.forca, math.sin(
            self.velocidade.ang) * self.velocidade.forca
        self.x = self.x + vx * t
        self.y = self.y + vy * t
        # Aplicando a aceleracao
        if self.aceleracao is not None:
            ax, ay = math.cos(self.aceleracao.ang) * self.aceleracao.forca, math.sin(
                self.aceleracao.ang) * self.aceleracao.forca
            vx2 = vx + ax * t
            vy2 = vy + ay * t
...
class Criatura(objeto_fisico):
    ...
    def __repr__(self):
        retorno = '<Criatura - Nome: %s, Genoma: %s, Velocidade: %s, X: %.3f, Y:%.3f >' % (
            self.nome, self.genoma, self.velocidade, self.x, self.y)
        return retorno
