...
HEIGHT = 400
TEMPOINCREM = 0.1
# Posicao do alvo: p(700,385) até p(705,390)
ALVO = (WIDTH - 100, HEIGHT - 15, 5, 5)
XINIT = 0
YINIT = 0
...
def desenharEixos():
    ...
    draw.rect(DISPLAYSURF, BLUE, ((0, HEIGHT),
                                  (10, HEIGHT - 10)), 20)  # Origem
...
def desenhar(pontos, rapido=False):
    if DESENHAR_RAPIDO:
        ...
        if rapido:
            ...
            try:
                draw.lines(DISPLAYSURF, COR_TRAJETORIA,
                           False, pontosFormat[:-1], 3)
            except:
                pass
            finally:
                time.wait(TICK)
